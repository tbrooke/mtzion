(ns com.mtzion
  (:require [com.biffweb :as biff]
            [com.mtzion.feat.app :as app]
            [com.mtzion.feat.auth :as auth]
            [com.mtzion.feat.home :as home]
            [com.mtzion.feat.worker :as worker]
            [com.mtzion.schema :refer [malli-opts]]
...
(defn generate-assets! [sys]
  (when (:com.mtzion/enable-web sys)
...
(defn on-save [sys]
  (biff/eval-files! sys)
  (generate-assets! sys)
  (test/run-all-tests #"com.mtzion.test.*"))

(defn start []
  (biff/start-system
    {:com.mtzion/chat-clients (atom #{})
     ...
     :biff/components [biff/use-config
                       ...
                       (biff/use-when
                         :com.mtzion/enable-web
                         biff/use-outer-default-middleware
                         biff/use-jetty)
                       (biff/use-when
                         :com.mtzion/enable-worker
                         biff/use-chime)
                       (biff/use-when
                         :com.mtzion/enable-hawk
                         biff/use-hawk)]})
